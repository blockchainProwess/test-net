...
approveForMyOrg() {
  ...
  setGlobals $ORG
  set -x
  peer lifecycle chaincode approveformyorg -o localhost:7050 --ordererTLSHostnameOverride orderer.pscm.com --tls --cafile $ORDERER_CA --channelID $CHANNEL_NAME --name ${CC_NAME} --version ${CC_VERSION} --package-id ${PACKAGE_ID} --sequence ${CC_SEQUENCE} ${INIT_REQUIRED} ${CC_END_POLICY} ${CC_COLL_CONFIG} >&log.txt
  res=$?
  { set +x; } 2>/dev/null
  cat log.txt
...
commitChaincodeDefinition() {
  ...
  set -x
  peer lifecycle chaincode commit -o localhost:7050 --ordererTLSHostnameOverride orderer.pscm.com --tls --cafile $ORDERER_CA --channelID $CHANNEL_NAME --name ${CC_NAME} $PEER_CONN_PARMS --version ${CC_VERSION} --sequence ${CC_SEQUENCE} ${INIT_REQUIRED} ${CC_END_POLICY} ${CC_COLL_CONFIG} >&log.txt
  res=$?
  { set +x; } 2>/dev/null
  cat log.txt
...
chaincodeInvokeInit() {
  ...
  peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.pscm.com --tls --cafile $ORDERER_CA -C $CHANNEL_NAME -n ${CC_NAME} $PEER_CONN_PARMS --isInit -c ${fcn_call} >&log.txt
  res=$?
  { set +x; } 2>/dev/null
  cat log.txt
...
packageChaincode Producer

## Install chaincode on peer0.Producer and peer0.Processor
infoln "Installing chaincode on peer0.Producer..."
installChaincode Producer
...
queryInstalled Producer

## approve the definition for Producer
approveForMyOrg Producer

## check whether the chaincode definition is ready to be committed
## expect Producer to have approved and Processor not to
checkCommitReadiness Producer "\"ProducerMSP\": true" "\"ProcessorMSP\": false" 
checkCommitReadiness Processor "\"ProducerMSP\": true" "\"ProcessorMSP\": false" 
...
checkCommitReadiness Producer "\"ProducerMSP\": true" "\"ProcessorMSP\": true"
checkCommitReadiness Processor "\"ProducerMSP\": true" "\"ProcessorMSP\": true" 

approveForMyOrg Distributor

approveForMyOrg Retailer
## now that we know for sure both orgs have approved, commit the definition
commitChaincodeDefinition Producer Processor Distributor Retailer

## query on both orgs to see that the definition committed successfully
queryCommitted Producer
...
if [ "$CC_INIT_FCN" = "NA" ]; then
  ...
  chaincodeInvokeInit Producer Processor Distributor Retailer
fi

...
function printHelp() {
  ...
  println "    -ccep <policy>  - Optional, chaincode endorsement policy, using signature policy syntax. The default policy requires an endorsement from Producer and "
...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Create Processor Identities"
  ...
  while :
    do
      if [ ! -f "organizations/fabric-ca/Producer/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
  ...
  infoln "Generate CCP files for Producer,Processor,Distributor AND Retailer"
...
## call the script to join create the channel and join the peers of Producer, Processor, Distributor and Retailer
...
function networkDown() {
  # stop Distributor containers also in addition to Producer, Processor, Distributor and Retailer in case we were running sample to add Distributor
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf addDistributor/fabric-ca/Distributor/msp addDistributor/fabric-ca/Distributor/tls-cert.pem addDistributor/fabric-ca/Distributor/ca-cert.pem addDistributor/fabric-ca/Distributor/IssuerPublicKey addDistributor/fabric-ca/Distributor/IssuerRevocationPublicKey addDistributor/fabric-ca/Distributor/fabric-ca-server.db'
...
# use this as the docker compose couch file for Distributor
COMPOSE_FILE_COUCH_Distributor=addDistributor/docker/docker-compose-couch-Distributor.yaml
# use this as the default docker-compose yaml definition for Distributor
